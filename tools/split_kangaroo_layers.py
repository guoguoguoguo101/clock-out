"""Chroma-key magenta kangaroo gens, split cyan scarf, write 1024 body + scarf PNGs."""
...
BODY_OUT = ROOT / "assets" / "game" / "chars" / "kangaroo"
SCARF_OUT = BODY_OUT / "scarf"
OLD_OUT = ROOT / "assets" / "废弃素材" / "chars" / "kangaroo_baked_scarf"
CANVAS = 1024
BOTTOM_PAD = 48
POSES = [f"{anim}_{i}" for anim in ("idle", "walk", "run", "work", "sleep", "toilet", "ride") for i in range(4)]
YELLOW = np.array([244.0, 193.0, 74.0], np.float32)
...
def fill_scarf_hole(rgb: np.ndarray, alpha: np.ndarray, scarf: np.ndarray) -> np.ndarray:
    ...
    r, g, b = rgb[:, :, 0], rgb[:, :, 1], rgb[:, :, 2]
    fur = body & (r > 150) & (g > 90) & (r > b + 20)
    if fur.any():
        fill = np.median(rgb[fur], axis=0)
    elif body.any():

...
var lunge_stun := 0.0
var lunge_hit := false
var audit_hit_left := 0.0
var audit_hit_dir := Vector2.DOWN
var power_pips := 0
var report_cd := 0.0
var fan_cd := 0.0
var throw_flash := 0.0
...
func _update_visual(delta := 0.0) -> void:
	if body_sprite == null:
		return
	_anim_acc += delta
	audit_hit_left = maxf(0.0, audit_hit_left - delta)
	if velocity.x > 10.0:
		_facing.x = 1.0
	...
	if audit_hit_left > 0.0:
		var hit_t := 1.0 - audit_hit_left / Rules.AUDIT_HIT_TIME
		var recoil := sin(hit_t * PI) * Rules.AUDIT_HIT_RECOIL
		body_sprite.position += audit_hit_dir * recoil
		body_sprite.rotation = -audit_hit_dir.x * sin(hit_t * PI) * 0.12
		body_sprite.scale *= Vector2(1.0 + sin(hit_t * PI) * 0.06, 1.0 - sin(hit_t * PI) * 0.05)
...
func show_audit_hit(from_dir: Vector2) -> void:
	if kind != Rules.Kind.EMPLOYEE:
		return
	audit_hit_dir = from_dir.normalized() if from_dir.length() > 0.12 else Vector2.DOWN
	audit_hit_left = Rules.AUDIT_HIT_TIME
	say("合规审查！", 0.72)
...
func apply_snapshot(px: float, py: float, st: int, h: float, e: float, vis: bool, mcd: float, kcd: float, dcd: float, talk: float = 0.0, rescue: float = 0.0, bike: float = 0.0, slow: float = 0.0, rcd: float = 0.0, fcd: float = 0.0, power: int = 0, lstun: float = 0.0, flyl: float = -1.0, flyc: float = -1.0, review: int = 0, review_window: float = 0.0, review_delay: float = 0.0) -> void:
	...
	talk_progress = talk
	review_step = review
	review_window_left = review_window
	review_delay_left = review_delay
	rescue_left = rescue
	bike_left = bike
	slow_left = slow
	report_cd = rcd

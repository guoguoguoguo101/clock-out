...
func tick(delta: float) -> void:
	...
	if door != null and (door.closed or door.opening):
		...
		return
	if actor.power_pips >= Rules.TIGER_POWER_MAX and wait <= 0.0:
		var mark: Actor = Match.meeting_target(actor)
		if mark != null:
	...
	if prey != null:
		...
		actor.input_dir = d.normalized()
		actor._facing = actor.input_dir
		if d.length() < 118.0:
			actor.want_interact = true
		if wait <= 0.0 and d.length() < 280.0 and d.length() > 90.0:
			if d.length() > 150.0 and actor.fan_cd <= 0.0:
				actor.want_fan = true
			else:

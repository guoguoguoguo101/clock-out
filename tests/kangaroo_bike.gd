...
func run_checks() -> void:
	...
	game._pick_slot(rules.Slot.EMP_C)
	game._enter_test_room()
	await physics_frame
	match_node.bots.clear()
	var rider = match_node.actors[rules.Slot.EMP_C]
	var horse = match_node.actors[rules.Slot.EMP_A]
	check(rider.skin == rules.CharSkin.KANGAROO, "EMP_C is kangaroo")
	check(game.office.points.has("seat_3"), "third desk exists")
	rider._stand_up()
	horse._stand_up()
